...
# Below are all of my additional imports
import photos
    ...
    def mass(self, event):
        ...
        if (event.totalWeight > 2):
            ...
            if self._measureCnt == WEIGHT_SAMPLES:
                ...
                self._measureCnt = 0
                # Getting weight reading
                weight = str(self._weight) + " lbs"
                # Getting timestamp
                timestamp = datetime.datetime.now().strftime("%d-%b-%Y")
                print (timestamp)
                print (weight)
                # Post these results to firebase
                post = firebase.post('/weight/', {'weight':weight, 'date':timestamp})
                print (post)
                print ("Now taking the photos!")
                # Start the takePhotos() method in photos.py
                photos.takePhotos()
                sys.exit()
